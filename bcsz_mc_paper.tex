...
Following the standard Galerkin procedure, we take the spatial moment of \eqt{eq:exact_transport} with respect to basis function $\B{i}(x,y)$  by first multiplying by basis function $\B{i}(x,y)$  and then integrating over the spatial cell $K$.
Assuming cell-wise constant $\sigma_t$, the $i-\text{th}$ spatial moment equation is:
\benum
\label{eq:mom_ex}
...
\eeanum
In this work, we use mapped, bilinear Lagrange interpolatory functions as our basis functions:
\begin{subequations}
\label{eq:basis_func}
\beanum
...
\eeanum
\end{subequations}
As a result, following the Galerkin procedure creates four equations with many more unknowns than equations.
To solve the exact moment equations, one must assume a solution representation, $\widetilde{\psi}(s,t,\omg_d)$, to approximate the true angular flux, $\psi(s,t,\omg_d)$.
