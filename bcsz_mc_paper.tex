...
As a first step towards accurate methods for radiative transfer on quadrilaterals, we seek a new, non-negative, bilinear discontinuous (BLD) finite element spatial discretization of the neutron transport equation for quadrilaterals.
...
+ \sigma_t\int_R{ \B{i}(x,y) \psi(x,y,\omg_d)~dxdy} \pep
\end{multline}
...
To solve the moment equations, one must assume a solution representation, $\widetilde{\psi}(s,t,\omg_d)$, to approximate the true angular flux, $\psi(s,t,\omg_d)$.
...
Though the BCSZ solution exhibits some numerical diffusion in \fig{fig:unstructured}, it is a result of the mesh, as the UBLD scheme exhibits similar increases in numerical diffusion.  Again, the UBLD solution exhibits negativities and oscillations and BCSZ remains strictly non-negative.
